...
const BookAppointment = ({date}) => {
    ...
    return (
        <section className='pb-5 text-center text-info'>
            <h2>Available Appointment on {date.toDateString()}</h2>
            <div className="container">
                <div className="row mt-5 margin-top">
                    {
                        bookingData.map(booking=><BookingCard  bookingData={booking} date={date} key={booking.id}></BookingCard>)
                    }
                </div>
            </div>
        </section>
